...
final class DataSourceTests: XCTestCase {
    ...
    func test_index_titles_returns_nil_when_disabled() {
        ...
        let banners = makeSampleBanners(5)
        let cellInfos = makeCellInfos(from: banners)
        cellInfos[0].indexTitle = "A"
        cellInfos[1].indexTitle = "B"

        let section = SSCollectionViewModel.SectionInfo(items: cellInfos)
        let viewModel = SSCollectionViewModel(sections: [section], isIndexTitlesEnabled: false)
        cv.ss.setViewModel(with: viewModel)
        cv.reloadData()

        // When
        let indexTitles = cv.dataSource?.indexTitles?(for: cv)
    ...
    func test_index_titles_returns_titles_when_enabled() {
        ...
        let cellInfos = makeCellInfos(from: banners)
        cellInfos[0].indexTitle = "A"
        cellInfos[1].indexTitle = "A"
        cellInfos[2].indexTitle = "B"
        cellInfos[3].indexTitle = "C"

        let section = SSCollectionViewModel.SectionInfo(items: cellInfos)
        let viewModel = SSCollectionViewModel(sections: [section], isIndexTitlesEnabled: true)
        cv.ss.setViewModel(with: viewModel)
        cv.reloadData()
        ...
        XCTAssertEqual(indexTitles?.count, 3, "Should contain unique index titles")
        XCTAssertTrue(indexTitles?.contains("A") ?? false)
        XCTAssertTrue(indexTitles?.contains("B") ?? false)
        XCTAssertTrue(indexTitles?.contains("C") ?? false)
    }
}
