...
//
//  Created by SunSoo Jeon on 2024/06/11.
//
...
final class PageBasedLoadingTests: XCTestCase {
    ...
    func test_load_multiple_pages_via_presenter() {
        ...
        _ = cv.ss.loadPage(0, hasNext: true) { builder in
            builder.section("TopBanner") { builder.cells(makeSampleBanners(2), cellType: TestBannerCell.self) }
            builder.section("ProductList") { builder.cells(makeSampleBanners(4), cellType: TestBannerCell.self) }
        }

        // When
        let result = cv.ss.loadPage(1, hasNext: false) { builder in
            builder.section("ProductList") { builder.cells(makeSampleBanners(6), cellType: TestBannerCell.self) }
        }

        // Then
        XCTAssertEqual(result.count, 2)
        XCTAssertEqual(result[0].count, 2, "TopBanner section unchanged")
    ...
    func test_remove_page_via_presenter() {
        ...
        let cv = makeCollectionView()
        _ = cv.ss.loadPage(0, hasNext: true) { builder in
            builder.section("TopBanner") { builder.cells(makeSampleBanners(3), cellType: TestBannerCell.self) }
        }
        _ = cv.ss.loadPage(1, hasNext: false) { builder in
            builder.section("TopBanner") { builder.cells(makeSampleBanners(5), cellType: TestBannerCell.self) }
        }
    ...
    func test_load_page_after_reset_starts_fresh() {
        ...
        let cv = makeCollectionView()
        _ = cv.ss.loadPage(0, hasNext: true) { builder in
            builder.section("ProductList") { builder.cells(makeSampleBanners(5), cellType: TestBannerCell.self) }
        }
        _ = cv.ss.loadPage(1, hasNext: false) { builder in
            builder.section("ProductList") { builder.cells(makeSampleBanners(5), cellType: TestBannerCell.self) }
        }
