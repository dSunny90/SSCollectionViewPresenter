...
final class PaginationAndPagingTests: XCTestCase {
    ...
    func test_set_paging_enabled_configures_presenter() {
        ...
        // Then
        XCTAssertTrue(cv.presenter?.isCustomPagingEnabled ?? false)
        XCTAssertTrue(cv.presenter?.isAlignCenter ?? false)
        XCTAssertTrue(cv.presenter?.isLooping ?? false)
        XCTAssertTrue(cv.presenter?.isInfinitePage ?? false)
        XCTAssertTrue(cv.presenter?.isAutoRolling ?? false)
        XCTAssertEqual(cv.presenter?.pagingTimeInterval, 2.5)
    }
    ...
    func test_disabling_paging_resets_dependent_flags() {
        ...
        // Then
        XCTAssertFalse(cv.presenter?.isCustomPagingEnabled ?? true)
        XCTAssertFalse(cv.presenter?.isAlignCenter ?? true)
        XCTAssertFalse(cv.presenter?.isInfinitePage ?? true)
        XCTAssertFalse(cv.presenter?.isAutoRolling ?? true)
    }
