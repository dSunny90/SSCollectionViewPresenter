...
final class PaginationAndPagingTests: XCTestCase {
    ...
    func test_on_prefetch_callback_is_stored() {
        // Given
        let cv = makeCollectionView()
        var prefetchedPaths: [IndexPath]?

        // When
        cv.ss.onPrefetch { paths in
            prefetchedPaths = paths
        }
        cv.presenter?.prefetchBlock?([IndexPath(item: 0, section: 0)])

        // Then
        XCTAssertNotNil(prefetchedPaths)
        XCTAssertEqual(prefetchedPaths?.count, 1)
    }

    func test_on_cancel_prefetch_callback_is_stored() {
        // Given
        let cv = makeCollectionView()
        var cancelledPaths: [IndexPath]?

        // When
        cv.ss.onCancelPrefetch { paths in
            cancelledPaths = paths
        }
        cv.presenter?.cancelPrefetchBlock?([IndexPath(item: 1, section: 0)])

        // Then
        XCTAssertNotNil(cancelledPaths)
        XCTAssertEqual(cancelledPaths?.count, 1)
    }
