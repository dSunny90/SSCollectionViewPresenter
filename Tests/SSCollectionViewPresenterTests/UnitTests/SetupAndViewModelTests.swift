...
final class SetupAndViewModelTests: XCTestCase {
    ...
    func test_selected_items_updated_when_cell_selected() {
        // Given
        let cv = makeCollectionView()
        _ = cv.ss.buildViewModel { builder in
            builder.section {
        ...
        // When
        guard cv.cellForItem(at: indexPath) is TestBannerCell else {
        ...
        // Then
        XCTAssertFalse(cv.ss.selectedItems.isEmpty)
        XCTAssertEqual(cv.ss.selectedItems.count, 1)
    }
    ...
    func test_clear_selected_items_removes_all_selections() {
        // Given
        let cv = makeCollectionView()
        _ = cv.ss.buildViewModel { builder in
            builder.section {
