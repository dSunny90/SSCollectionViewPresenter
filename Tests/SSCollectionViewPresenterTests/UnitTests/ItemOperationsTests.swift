...
final class ItemOperationsTests: XCTestCase {
    ...
    func test_append_items_by_section_identifier() {
        ...
        let cv = makeCollectionView()
        _ = cv.ss.buildViewModel { builder in
            builder.section("products") { builder.cells(makeSampleBanners(2), cellType: TestBannerCell.self) }
        }
        let items = makeCellInfos(from: makeSampleBanners(5))

        // When
        cv.ss.appendItems(contentsOf: items, sectionIdentifier: "products")
    ...
    func test_append_item_to_last_section() {
        ...
        let cv = makeCollectionView()
        _ = cv.ss.buildViewModel { builder in
            builder.section("Swift") { builder.cells(makeSampleBanners(9), cellType: TestBannerCell.self) }
            builder.section("Objective-C") { builder.cells(makeSampleBanners(16), cellType: TestBannerCell.self) }
        }
    ...
    func test_remove_items_multi_section_correct_order() {
        ...
        let cv = makeCollectionView()
        _ = cv.ss.buildViewModel { builder in
            builder.section("Hi") { builder.cells(makeSampleBanners(11), cellType: TestBannerCell.self) }
            builder.section("Bye") { builder.cells(makeSampleBanners(5), cellType: TestBannerCell.self) }
        }
    ...
    func test_remove_all_items_in_section() {
        ...
        let cv = makeCollectionView()
        _ = cv.ss.buildViewModel { builder in
            builder.section("Cart") { builder.cells(makeSampleBanners(3), cellType: TestBannerCell.self) }
            builder.section("Todo") { builder.cells(makeSampleBanners(7), cellType: TestBannerCell.self) }
        }
    ...
    func test_move_item_across_sections() {
        ...
        let cv = makeCollectionView()
        _ = cv.ss.buildViewModel { builder in
            builder.section("LOL") { builder.cells(makeSampleBanners(4), cellType: TestBannerCell.self) }
            builder.section("TFT") { builder.cells(makeSampleBanners(3), cellType: TestBannerCell.self) }
        }
        ...
        // Then
        XCTAssertEqual(cv.ss.getViewModel()?[0].count, 3, "Source section shrinks")
        XCTAssertEqual(cv.ss.getViewModel()?[1].count, 4, "Destination section grows")
    }
    ...
    func test_move_item_destination_clamped_to_end() {
        ...
        let cv = makeCollectionView()
        _ = cv.ss.buildViewModel { builder in
            builder.section("HOS") { builder.cells(makeSampleBanners(4), cellType: TestBannerCell.self) }
            builder.section("Starcraft") { builder.cells(makeSampleBanners(2), cellType: TestBannerCell.self) }
        }

        // When — destination row exceeds count -> clamped to end
        cv.ss.moveItem(from: IndexPath(item: 0, section: 0), to: IndexPath(item: 99, section: 1))
    ...
    func test_item_count_in_section() {
        ...
        XCTAssertEqual(cv.ss.itemCount(inSection: 99), 0)
    }
