...
//
//  Created by SunSoo Jeon on 2021/04/24.
//
...
public final class SSCollectionViewPresenter: NSObject {
    ...
    /// Per-section `visibleItemsInvalidationHandler` closures, keyed by
    /// section index. A negative key (`-1`) is reserved for an
    /// "all sections" handler used when no per-section handler is set.
    ///
    /// Stored as `Any` to avoid `@available` annotations on stored properties;
    /// the actual type is ``VisibleItemsInvalidationHandler`` (iOS 13+).
    internal var visibleItemsInvalidationHandlers: [Int: Any] = [:]
            ...
            if #available(iOS 13.0, *) {
                if let config = config, collectionView.collectionViewLayout as? UICollectionViewCompositionalLayout == nil {
                    let layout = config.makeLayout(
                        viewModelProvider: { [weak self] in
                            guard let self = self else { return nil }
                            return self.viewModel
                        },
                        visibleItemsInvalidationHandlerProvider: { [weak self] sectionIndex in
                            guard let self = self else { return nil }
                            let handler = self.visibleItemsInvalidationHandlers[sectionIndex]
                                ?? self.visibleItemsInvalidationHandlers[-1]
                            return handler as? VisibleItemsInvalidationHandler
                        }
                    )
                    collectionView.setCollectionViewLayout(layout, animated: false)
                }
