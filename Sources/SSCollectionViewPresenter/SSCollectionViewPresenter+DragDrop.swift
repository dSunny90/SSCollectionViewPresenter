...
//
//  Created by SunSoo Jeon on 2024/06/08.
//
...
extension SSCollectionViewPresenter: UICollectionViewDropDelegate {
    ...
    private func moveCellInfos(
        ...
        if destination.section < newViewModel.count {
            adjustedDestinationItem = max(
                0,
                min(destination.item, newViewModel[destination.section].count)
            )
        } else {
        ...
        return movedItems.indices.map {
            IndexPath(item: adjustedDestinationItem + $0,
                      section: destination.section)
        }
    }
    ...
    private func insertExternalCells(_ cellInfos: [CellInfo],
        ...
        let indexPaths = cellInfos.indices.map {
            IndexPath(item: clampedStart + $0, section: indexPath.section)
        }
